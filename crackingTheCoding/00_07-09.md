...
Depending on the exact implementation of the "is this a permutation" part, you'll probably get a runtime of either O(B * S), O(B * S log S), or O(B * S²). None of these are the most optimal algorithm (there is an O(B) algorithm), but it's a lot better than what we had before.
