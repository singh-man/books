...
If we imagine our current algorithm's runtime asO(N  x N), then getting toO(N) orO(N  x  log N) might mean reducing that secondO(N) in the equation to O(1) or O(log N).
...
Our focus is still on reducing search from O(log N) to O(1). Any precomputation that's O(N) or less is
"free:'
